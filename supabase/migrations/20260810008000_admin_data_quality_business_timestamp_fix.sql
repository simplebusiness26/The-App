-- Admin Dashboard, Stage 7 follow-up: businesses do not have a created_at
-- column in the live schema. Keep owner-state findings in the report with a
-- null sort timestamp instead of assuming one exists.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      to_jsonb(issue)
      order by issue.created_at desc nulls last,issue.issue_id
    ),
    '[]'::jsonb
  )
