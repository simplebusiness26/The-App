...
// Alex challenger navigation dock.
//
// This deliberately throws away the frozen pale footer + floating circular Map
// button. The dock is one dark contextual object: Explore / Now / Map / Inbox /
// You. Map owns the centre position, and when the Explorer is already on Map the
// same centre position becomes the existing Camera action.

const ICONS={
  compass:[
    {circle:[8,8,5.5]},
    {path:"M10.8 5.2 9.3 9.3 5.2 10.8 6.7 6.7z"}
  ],
  pulse:[
    {path:"M1.8 8h2.7l1.4-3.1 2.2 6.2 1.7-4.2 1.1 1.1h3.3"}
  ],
  map:[
          ...
          return(
            ...
            >
              <View style={[styles.iconBox,isActive && styles.iconBoxActive,isCentre && styles.centreIconBox]}>
                <Icon
                  name={item.glyph}
                  colour={isCentre ? INK.navy : isActive ? INK.brand : INK.onNavySoft}
                  size={isCentre ? 23 : 20}
                />
              </View>
              <Text style={[styles.label,isActive && styles.labelActive,isCentre && styles.centreLabel]} numberOfLines={1}>
                {label}
              </Text>
            </Pressable>
          );
