...
export default function Settings(){
  ...
  const [switchingManager,setSwitchingManager]=useState(false);
  ...
  const load=useCallback(async()=>{
    ...
    const {data:profile,error:profileError}=await supabase
      .from("profiles")
      .select("email,area,show_area,leaderboard_opt_in,is_manager")
      .eq("id",user.id)
      .maybeSingle();
    ...
    if(profile.is_manager){
      ...
      setCapabilities(capabilityRow || null);
    }else{
  ...
  async function applyManagerChange(enabled){
    setSwitchingManager(true);

    const {error:rpcError}=await supabase.rpc("set_manager_account",{p_enabled:enabled});

    setSwitchingManager(false);

    if(rpcError){
      showFeedback(rpcError.message,"error","Manager account unchanged");
      return;
    }

    showFeedback(
      enabled
        ? "Businesses and properties are ready to use. Activity clubs and events need to be requested from the dashboard."
        : "Your listings and capabilities have been kept, and switching it back on restores them.",
      "success",
      enabled ? "Manager account switched on" : "Manager account switched off"
    );

    await load();
  }

  function toggleManager(enabled){
    if(enabled){
      applyManagerChange(true);
      return;
    }

    Alert.alert(
      "Switch off your manager account?",
      "Your listings, claims and capabilities are kept, and your Explorer profile is unaffected. You will lose access to the manager dashboard until you switch it back on.",
      [
        {text:"Cancel",style:"cancel"},
        {text:"Switch off",style:"destructive",onPress:()=>applyManagerChange(false)}
      ]
    );
  }
  ...
  return(
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Text style={styles.helpText}>
        A manager account is added to this Explorer profile rather than replacing it.
        Everything you do as an Explorer keeps working exactly as it does now.
      </Text>

      <View style={styles.settingRow}>
        <View style={styles.settingTextWrap}>
          <Text style={styles.settingTitle}>I manage places or events</Text>
          <Text style={styles.settingText}>Unlocks the manager dashboard so you can list businesses and properties.</Text>
        </View>
        <Switch value={isManager} onValueChange={toggleManager} disabled={switchingManager}/>
      </View>
